...
if($_GET['board']==$iurlpart['host']) { $_GET['board'] = $Settings['root_board']; }
$_GET['board'] = strtolower($_GET['board']);
$_GET['board'] = preg_replace("/[^A-Za-z0-9_$]/", "", $_GET['board']);
$_GET['board'] = preg_replace("/(.*?)\.\/(.*?)/", "", $_GET['board']);
$_GET['board'] = preg_replace("/(.*?)\/(.*?)/", "", $_GET['board']);
$_GET['board'] = preg_replace("/(.*?)\.(.*?)/", "", $_GET['board']);
$Settings['idburl'] = $iurlpart['scheme']."://".$_GET['board'].".".$iurlpart['host'].$iurlpart['path'];
if($_GET['board'].".".$iurlpart['host']!=$_SERVER["HTTP_HOST"]) {
	header('Location: '.$Settings['idburl'].'index.php?act=view'); }
...
$Settings['idburl'] = $Settings['oldidburl'];
